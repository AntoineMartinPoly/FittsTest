...
{
    //private const string LARGE_COLORED_BRICKS_MATERIAL = "Materials/wall-846013";
    //private const string SMALL_COLORED_BRICKS_MATERIAL = "Materials/bricks-868344";
    //private const string SMALL_GREY_BRICKS_MATERIAL = "Materials/stone-805287";
    //private const string DAMAGED_BRICKS_MATERIAL = "Materials/tile-846018";
    ...
    {
        if (targets != null)
            return targetsColliding.Select(x => x.gameObject).Contains(targets[targetIndexToSelect]);
        return false;
    }
    ...
    {
        // We keep the initial position of the camera when the test begin so the targets are not created at different distance from the user throughout the test
